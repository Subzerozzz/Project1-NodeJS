...
const sider = document.querySelector(".sider");
if (sider) {
  //Lấy ra đường link ở trên website
  const url = window.location.pathname;
  const arraySplitUrl = url.split("/");
  const listUrl = document.querySelectorAll("a");
  listUrl.forEach((item) => {
    const href = item.href;
    const pathname = new URL(href).pathname;
    const listItemOfPathname = pathname.split("/");
    if (
      arraySplitUrl[1] == listItemOfPathname[1] &&
      arraySplitUrl[2] == listItemOfPathname[2]
    ) {
...
// Logout
const elementLogout = document.querySelector(".inner-logout");
